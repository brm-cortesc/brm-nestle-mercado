...
module.exports = function(grunt) {
	...
	//Se inicia la configuración del projecto
		...
		pkg: grunt.file.readJSON('casa-del-bienestar.jquery.json'),
		...
		concat: {
			...
			dist: {
				src: ['publication/js/libs/bootstrap.min.js','publication/js/libs/ripples.min.js','publication/js/libs/material.min.js', 'publication/js/libs/jquery.validate.js'],
		...
		stylus: {
			...
			compile: {
				files: {
					'publication/css/style.css': 'publication/stylus/main.styl'
				}
			}
		},
		...
		cssmin: {
		      combine: {
		      files: {
		        'publication/css/<%= pkg.name %>.min.css': ['publication/css/*.*', "!publication/css/casa-del-bienestar.min.css"]
		      }
		    }
		  },
		...
		jade: {
			 compile: {
					 options: {
							 pretty: true,
							 data:{
							 	debug: debug //Variable para compilar html con archivos de JS y CSS comprimidos 
							 }
					 },
					 files: [ {
		...
		watch: {
			jade: {
				...
				tasks : ["jade"],/*las tareas que se corren por defecto al observar cambios en los archivos*/
				task : ['shell:stats']
			},
			stylus: {
				
				files: ['publication/**/*.styl'],

				tasks : ["stylus", 'cssmin'],/*las tareas que se corren por defecto al observar cambios en los archivos*/
				task : ['shell:stats']
			},
		},
	});
